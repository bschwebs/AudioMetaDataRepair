...
Repairs metadata for MP3 and FLAC files by extracting information from filenames
and optional album.nfo files.
"""
...
def mark_album_art_downloaded(artist: str, album: str, log_data: Dict, success: bool):
...
def get_album_art(artist: str, album: str, musicbrainz_release_group_id: Optional[str] = None) -> Optional[bytes]:
    """
...
def repair_audio_file(file_path: Path, base_dir: Path, album_art_cache: Dict[str, Optional[bytes]], 
                      log_data: Dict, log_file: Path) -> bool:
    ...
    if artist and album:
        ...
        if has_album_art_downloaded(artist, album, log_data):
            ...
            if album_key not in album_art_cache:
                ...
                album_art_data = get_album_art(artist, album, musicbrainz_id)
                album_art_cache[album_key] = album_art_data
                # Mark in log that we attempted download
                mark_album_art_downloaded(artist, album, log_data, album_art_data is not None)
...
def main():
    ...
    for audio_file in sorted(audio_files):
        ...
        was_already_processed = is_file_processed(audio_file, log_data)
        result = repair_audio_file(audio_file, target_dir, album_art_cache, log_data, log_file)
        if result:
